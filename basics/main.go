...
func main() {
	...
	*addressOfP1 = Person{
	...
	// an interesting thing, you cannot use var while initializing i in golang
	// for var i = 0; i < 5; i++ {
	// 	fmt.Println(i)
	// }
	...
	// another interesting thing, slices are pointers and sub slices are pointers
	sBig := []int{1, 2, 3, 4, 5}
	sSmall := sBig[2:]
	fmt.Println(sBig, sSmall)
	sSmall[0] = 9999
	fmt.Println(sBig, sSmall) // sBig also changed :-0
	anotherPointerToSBig := sBig
	anotherPointerToSBig[1] = 8888
	fmt.Println(sBig, sSmall) // sBig also changed :-0
